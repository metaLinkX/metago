...
func GetEntityDescTagsFieldMap(entity interface{}) (tags []string, tagFiledMap map[string]string, err error) {
	var formRef = reflect.TypeOf(entity)
	tagFiledMap = make(map[string]string)
	for i := 0; i < formRef.NumField(); i++ {
		field := formRef.Field(i)
		if field.Type.Kind() == reflect.Struct {
			err = gerror.New("can include Struct")
			continue
		}
		tag := reflectTagName(field, descTags, true)
		tagFiledMap[tag] = field.Name
