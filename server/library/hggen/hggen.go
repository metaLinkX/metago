...
//go:linkname doGenDaoForArray server/library/hggen/internal/cmd/gendao.doGenDaoForArray
...
func GenTypeSelect(ctx context.Context) (res sysin.GenTypeSelects, err error) {
	for k, v := range consts.GenCodesTypeNameMap {
		...
		if ok {
			...
			err = g.Cfg().MustGet(ctx, "genCode.application."+confName+".templates").Scan(&temps)
			if err != nil {
				return
			}
			if len(temps) > 0 {
...
func DbSelect(ctx context.Context) (res form.Selects) {
	dbs := g.Cfg().MustGet(ctx, "genCode.selectDbs")
