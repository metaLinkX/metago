<template>
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="租户">
    ...
    <n-card :bordered="false" class="proCard">
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/hgUser/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/hgUser/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
          ...
          <n-button type="primary" @click="handleExport" class="min-left-space" v-if="hasPermission(['/hgUser/export'])">
            <template #icon>
              <n-icon>
                <ExportOutlined />
              </n-icon>
...
<script lang="ts" setup>
  ...
  import { List, Export, Delete } from '@/api/hgUser';
  import { PlusOutlined, ExportOutlined, DeleteOutlined } from '@vicons/antd';
  import {columns, schemas, setOptMap, State} from './model';
  ...
  const loadDataTable = async (res) => {
    const result = await List({ ...searchFormRef.value?.formModel, ...res });
    // todo
    if (result.dict) {
      setOptMap(result.dict)
